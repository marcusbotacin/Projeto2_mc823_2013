...
int useMemory(unsigned instr_anterior)
{
	...
	if(instr_anterior>=1 && instr_anterior<=7)
	{
	return 1;
	}else{
	return 0;
...
#undef AC_SYSC
	...
	if(useMemory(instr_anterior) && checkRegister())
		{
			cout<<"Hazard\n";
			hazard_count++;
		}

	instr_anterior=instr_atual;
	IE_rt=IF_rt;
	IE_rs=IF_rs;
